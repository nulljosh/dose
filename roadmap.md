...
- [x] Re-shoot the iOS fastlane screenshot set — done 2026-08-29, all 10 frames (6.5" 1242x2688 + 6.9" 1320x2868) re-captured and verified by eye against their filenames. **Staged locally, NOT uploaded**: iOS 2.3.5 is REJECTED (4.3(a), reply filed, do not resubmit) and macOS 2.3.5 is IN_REVIEW, so these go up with the next iOS version.
  - The note's "off by one step ... etc." was only half right. The damage was confined to the first two frames: `0Home` had the What's New sheet over it, and the tap meant for Library was consumed dismissing that sheet, so `1Library` showed Home and Library was never captured at all. `2Insights` onward were already correct.
  - Two root causes, both fixed. (1) `WhatsNewSheet` now suppresses itself under the `UITEST_SNAPSHOT` launch arg, matching the existing `isUITestSnapshot` idiom in `HealstackApp`. Gating on the launch arg rather than pinning a `whats_new_seen_version` value is deliberate: a pinned version silently stops suppressing the next time `whatsNewVersion` is bumped, which is how it leaked into the store set originally.
  - (2) Suppressing the sheet exposed a second, worse bug: with Library actually loading, the taps for Insights and Body both landed on the Library grid's Zinc card, so **two** frames became a substance detail page. The floating tab bar is drawn over scrollable content, so tapping it is inherently racy. `PreviewScreenshot` now launches a fresh app per screen with `-UITEST_TAB <n>` and `HealstackApp` seeds `selectedTab` from it — no taps, no hit-test race, deterministic regardless of what sits under the bar.
